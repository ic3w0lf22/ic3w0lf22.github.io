...
(function (root, name, factory) {
  ...
  function isKeyword(id) {
    switch (id.length) {
      ...
      case 8:
        return 'function' === id || (features.continueSupport && 'continue' == id);
    }
    return false;
  }
  ...
  function parseAssignmentOrCallStatement(flowContext) {
    ...
    var compoundSymbols = ['+', '-', '*', '/', '%', '^', '..'];
      ...
      if (token.value.substr(0, char.length) == char) {
        ...
        var exp = parseExpectedExpression(flowContext);
    
        pushLocation(marker);
        return finishNode(ast.compoundAssignmentStatement(targets[0], operator, exp));
      }
	  }
