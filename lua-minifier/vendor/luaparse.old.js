...
(function (root, name, factory) {
	...
	var ast = exports.ast = {
	  ...
	  , compoundAssignmentStatement: function(expression, operator, expected) {
		return {
	...
	function lex() {
	  ...
	  switch (charCode) {
		...
		// , { } ] ( ) ; & # |
		case 44: case 123: case 124: case 125:
	...
	function parseAssignmentOrCallStatement() {
	  ...
	  if (trackLocations) marker = createLocationMarker();
	  expression = parsePrefixExpression(); // console.log(expression, token.value);
	  ...
	  for (var i = 0; i < compoundSymbols.length; i++) {
		var char = compoundSymbols[i];
		
		if (token.value.substr(0, char.length) == char) {
		  var operator = token;
		  ...
		  pushLocation(marker);
		  return finishNode(ast.compoundAssignmentStatement(expression, operator, exp));
		}
	  }
